...
class taskController:
    ...
    def unfinished_tasks(self):
        ...
        with open(self.file_name, "r") as read_file:
            for line in read_file:
                ...
                if done:
                    continue
                unfinished.append({'title':title, 'description':description, 'start_date':start_date, 'end_date':end_date})
    ...
    def print_table(self, tasks):
        ...
        for number, task in enumerate(tasks, 1):
            ...
            table.append({'no. ': number, **task, 'due_date': due_date})

        print(tabulate(table, headers='keys'))

    def display(self, args):
        all_tasks = self.all_tasks()
        unchecked_tasks = self.unfinished_tasks()

        if not all_tasks:
            print('No tasks found. To add a task use add <Task> \n')
            return
